...
"""
    MStestRes{reg::ArrayReg, witnessOp::PutBlock, overlap::Float64}
Fields:
\n`reg::ArrayReg`: The register goes through `MStest`. nqubits(MStest.reg) = 1 + vBit + rBit + nBitT.
...
"""
function MStest(regT::ArrayReg, MSCircuit::ChainBlock; nMeasure::Int64=1, useCuYao::Bool=CUDA_ON)
...
"""
    ...
    MStestRes{reg::ArrayReg, witnessOp::PutBlock, overlap::Float64}
Method 2 of `MStest`:
\n `regAll::ArrayReg`: Input register(state) for MSCircuit(MSQR circuit) built by `MScircuit`. `regAll = join(zero_state(1+nBitV+nBitR), regT)`
"""
function MStest(MSCircuit::ChainBlock; regAll::ArrayReg)  
    regA = copy(regAll)
    nBitA = nqubits(regA)
...
"""
function MSTtest(regT::ArrayReg, MSCircuit::ChainBlock, cExtend::ChainBlock; nMeasure::Int64=1, useCuYao::Bool=CUDA_ON)             
...
"""
    nMcheck(nMeasure::Int64, regs::ArrayReg...)
Function that check if the user not only define nMeasure but also define nbatch.  
"""
function nMcheck(nMeasure::Int64, regs::ArrayReg...)
